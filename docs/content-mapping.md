...
首阶段记录日期：2026-09-04，以下39节点／63选项映射描述序章至第二章子集。2026-09-07当前包为 `v0.7C-season-one.20260906.1`，462节点／251存储选项，覆盖序章至第十三章，不含AI。最新来源、条件与差异见[全季映射](第十至第十三章内容接入映射.md)。以下来源哈希、行号与旧正文修正是首次接入历史，非当前快照。新增内容见[第三至第五章映射](第三至第五章内容接入映射.md)。

## 可复现来源

`npm run content:build` 只读取本工程 `content-source/v0.4B.md`、`content-source/v0.5D.md`、`content-source/v0.7C-ch6-ch9.md` 、`content-source/v0.7C-ch10-ch13.md` 和 `content-source/baseline-v0.7C.json`，输出 `src/content/stage-one.json`、`content-source/state-mapping.json`。运行不读取父目录、旧项目或网络。
