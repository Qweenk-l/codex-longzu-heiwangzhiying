...
章节测试通过“打开章节列表”进入右侧抽屉。序章至第二章可测试，第三至第五章暂列“待接入”；后续实际进入运行包的章节自动显示为可测试项。列表独立滚动，可通过关闭按钮、Esc或遮罩关闭。
...
自动存档受当前浏览器存储条件影响。清除网站数据会删除本地进度；导出文件可在另一浏览器通过导入恢复。当前运行包 .5 支持本程序 .1/.2/.3/.4 的已知正文及反馈修订升级；不兼容旧游戏存档或任意剧情版本。
