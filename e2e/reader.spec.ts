...
test('chapter drawer scrolls in a short viewport and opens all implemented chapters',async({page},info)=>{
  ...
  await trigger.click();
  const drawer=page.getByRole('dialog',{name:'章节测试',exact:true});
...
test('fixed reading areas, safe free input, saved progress and isolated chapter tests',async({page},info)=>{
  ...
  await page.getByRole('button',{name:new RegExp(story.chapters[2].title)}).click();
...
test('each choice opens a complete new page, history is read-only and survives reload',async({page},info)=>{
  ...
  await expect(page.locator('.story-region p').filter({hasText:/^门外，婶婶的敲门声一阵紧过一阵。敲到第三回，门板已经有了投降的意思。$/})).toHaveCount(1);
